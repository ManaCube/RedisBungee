...
public class RedisVelocityAPI {
     ...
     *
     * @param player a player name
     * @return a {@link com.velocitypowered.api.proxy.server.ServerInfo} for the server the player is on.
     */
    public final ServerInfo getServerFor(@NonNull UUID player) {
        String server = plugin.getDataManager().getServer(player);
        return plugin.getProxy().getServer(server).map((RegisteredServer::getServerInfo)).orElse(null);
    }
