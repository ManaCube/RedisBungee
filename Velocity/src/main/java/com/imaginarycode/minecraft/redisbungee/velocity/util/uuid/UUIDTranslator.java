...
public final class UUIDTranslator {
    ...
    public final UUID getTranslatedUuid(@NonNull String player, boolean expensiveLookups) {
        ...
        Optional<Player> playerOpt = plugin.getProxy().getPlayer(player);

        if (playerOpt.isPresent())
            return playerOpt.get().getUniqueId();
        ...
        try (Jedis jedis = plugin.getPool().getResource()) {
            ...
            if (!expensiveLookups || !plugin.getProxy().getConfiguration().isOnlineMode())
                return null;
    ...
    public final String getNameFromUuid(@NonNull UUID player, boolean expensiveLookups) {
        ...
        if (playerOpt.isPresent())
            return playerOpt.get().getUsername();
        ...
        try (Jedis jedis = plugin.getPool().getResource()) {
            ...
            try {
                ...
                plugin.getLogger().warn("Unable to fetch name from Mojang for " + player, e);
                return null;
            }
            ...
            plugin.getLogger().warn("Unable to fetch name for " + player, e);
            return null;
        }
    }
